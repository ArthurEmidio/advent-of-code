...
int solve(const string &curr, set<string> &visited, const Graph &graph) {
    if (curr == "start") return 1;
    if (visited.count(curr) && isSmallCave(curr)) return 0;

    bool added = false;
    if (!visited.count(curr)) visited.insert(curr), added = true;

    int ans = 0;
    for (const string &adj : graph.at(curr)) ans += solve(adj, visited, graph);
...
int solve(const Graph &graph) {
    set<string> visited;
    return solve("end", visited, graph);
}
